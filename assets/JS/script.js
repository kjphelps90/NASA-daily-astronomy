...
var nearEarthObjectsCall = "https://api.nasa.gov/neo/rest/v1/feed?api_key=" + APIKey + "&start_date=" + CURRENTDAY + "&end_date=" + CURRENTDAY;
...
function createTable(nearEarthData, queryDate){
  ...
  for (let i=0; i < earthObjectsArray.length; i++) {
      ...
      var objectSize = Math.round(earthObjectsArray[i].estimated_diameter.meters.estimated_diameter_max) + " m";
      console.log(objectSize);
      var objectVelocity = Math.round(earthObjectsArray[i].close_approach_data[0].relative_velocity.kilometers_per_hour) + " km/h";
      console.log(objectVelocity);
      var objectMissDistance = Math.round(earthObjectsArray[i].close_approach_data[0].miss_distance.kilometers) + " km";
